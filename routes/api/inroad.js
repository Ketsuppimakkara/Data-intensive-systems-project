...
const pool = require("../../inroad_database")
var router = express.Router();

/* GET inroad data from database. */
...
router.post('/add/',async function(req, res, next) {
    try{
        const client = await pool.connect();
        await client.query('PREPARE query (numeric,numeric,numeric,numeric,text,numeric,numeric) AS INSERT INTO OBSERVATION VALUES ($1,$2,$3,$4,CURRENT_TIMESTAMP,$5,$6,$7);');
        result = await client.query('EXECUTE query ('+req.get("id")+','+req.get("sensorid")+','+req.get("latitude")+','+req.get("longitude")+',\''+req.get("vehicleclass")+'\','+req.get("vehiclelength")+','+req.get("vehiclespeed")+');');
        await client.query('DEALLOCATE query');
        client.release();
