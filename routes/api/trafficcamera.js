...
router.post('/add/',async function(req, res, next) {
    try{
        const client = await pool.connect();
        await client.query('PREPARE query (numeric,numeric,numeric,numeric,numeric) AS INSERT INTO OBSERVATION VALUES ($1,$2,$3,$4,CURRENT_TIMESTAMP,$5);');
        result = await client.query('EXECUTE query ('+req.get("id")+','+req.get("sensorid")+','+req.get("latitude")+','+req.get("longitude")+','+req.get("vehiclesinview")+');');
        await client.query('DEALLOCATE query');
        client.release();
